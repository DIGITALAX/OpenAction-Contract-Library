...
export class CollectionCreated extends Entity {
  ...
  set printType(value: string | null) {
    if (!value) {
      this.unset("printType");
    } else {
      this.set("printType", Value.fromString(<string>value));
    }
  }
...
export class CommunityCreated extends Entity {
  ...
  get validPrintTypes(): Array<string> | null {
    let value = this.get("validPrintTypes");
    if (!value || value.kind == ValueKind.NULL) {
      return null;
    } else {
      return value.toStringArray();
  ...
  set validPrintTypes(value: Array<string> | null) {
    if (!value) {
      this.unset("validPrintTypes");
    } else {
      this.set("validPrintTypes", Value.fromStringArray(<Array<string>>value));
    }
  }
...
export class PrintTypeCollectionRegistry extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    ...
    assert(
      id != null,
      "Cannot save PrintTypeCollectionRegistry entity without an ID"
    );
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type PrintTypeCollectionRegistry must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set(
  ...
  static load(id: Bytes): PrintTypeCollectionRegistry | null {
    return changetype<PrintTypeCollectionRegistry | null>(
      store.get("PrintTypeCollectionRegistry", id.toHexString())
    );
  }
